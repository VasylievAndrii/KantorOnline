...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  try {
    ...
    $insertPurchaseQuery = "INSERT INTO purchased_crypto (user_id, currency, amount, purchase_date, transaction_id) VALUES (?, ?, ?, NOW(), ?)";
    $transactionId = uniqid("txn_");
    $stmt = mysqli_prepare($conn, $insertPurchaseQuery);
    mysqli_stmt_bind_param($stmt, "isds", $userId, $currency, $amount, $transactionId);
    mysqli_stmt_execute($stmt);

    // Проверка наличия криптовалюты в кошельке и обновление данных
    ...
    echo json_encode(["status" => "success", "message" => "Kryptowaluta została pomyślnie zakupiona", "new_balance" => $newBalance]);
  } catch (Exception $e) {
